= Nether
Nether is an engine for easily adding endless page functionality to content paginated 
...
The number of pages, the path to get more content and the css id of the dom object to render. 
There is also an optional fourth argument for specifying the dom object containing the content 
to be rendered.
...
== Usage (kaminari)
Usage is the same as for will_paginate, however the method num_pages is instead used to return the 
number of pages.
